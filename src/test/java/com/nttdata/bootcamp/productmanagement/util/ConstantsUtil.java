package com.nttdata.bootcamp.productmanagement.util;

/**
 * Clase para almacenar constantes por defecto.
 */
public class ConstantsUtil {
    public static final String defaulEncodingApi = "UTF-8";

    // #region Constantes para archivos json
    public static final String findAllActiveProductsMock_Success = 
        "ProductsJson/FindAllActiveProductsMock.json";
    public static final String createActiveProductMock_Success = 
        "ProductsJson/CreateActiveProductMock.json";
    public static final String findActiveProductByIdMock_Success = 
        "ProductsJson/FindActiveProductByIdMock.json";
    public static final String findActiveProductsByClientIdMock_Success = 
        "ProductsJson/FindActiveProductsByClientIdMock.json";
    public static final String updateProductMock_Success = "ProductsJson/UpdateProductMock.json";
    // #endregion
}
